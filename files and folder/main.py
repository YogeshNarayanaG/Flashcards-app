...
@app.route('/review/<deckname>',methods=['GET','POST'])
def review(deckname):
    ...
    cur.execute(query,(deckname,))
    rows_global=cur.fetchone()
    global req_sno
    req_sno= rows_global[0]
    if rows_global is not None and request.method=="GET":
        return render_template('review.html',deckname=deckname,rows=rows_global)
...
@app.route('/review/<deckname>/score',methods=['GET','POST'])
def score(deckname):
    ans=0
    if request.method=='GET':
        conn=sqlite3.connect("project.db")
        cur=conn.cursor()
        query="""SELECT hardness from cards WHERE deckname=?"""
        cur.execute(query,(deckname,))
        r=cur.fetchall()
        for k in r:
            ans+=int(k[0])
        cur.execute("INSERT INTO score values (?) WHERE deckname=?",(ans,deckname))
        return render_template('score.html',score=ans) 

@app.route('/review/<deckname>/add_score',methods=['GET','POST'])
def add_score(deckname):
    if request.method=='POST':
        hv=request.form['hv']
        conn=sqlite3.connect("project.db")
        cur=conn.cursor()
        query="""UPDATE cards SET hardness=? WHERE deckname=? AND sno=?"""
        cur.execute(query,(hv,deckname,req_sno))
        conn.commit()

        return review(deckname)
